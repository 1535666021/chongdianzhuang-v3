          ...
          {orders.length === 0 ? <p className="py-10 text-center text-sm text-gray-400">暂无已完成订单</p> : (
            <div className="min-w-[560px] text-xs">
              ...
              {orders.map((order) => {
                ...
                return <div key={order.id} className="grid grid-cols-[1.2fr_1fr_1fr_1fr_1fr] gap-2 border-t border-gray-100 px-4 py-3 text-gray-700"><span className="truncate">{order.customerName}</span><span className="truncate">{order.platformName || order.platform}</span><span className="text-right">{formatMoney(order.customerPrice || 0)}</span><span className="text-right">{formatMoney((order.materialCost || 0) + (order.laborCost || 0))}</span><span className="text-right text-green-600">{formatMoney(profit)}</span></div>
              })}
            </div>
          )}
        </div>
